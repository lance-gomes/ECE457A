...
# basically just get all combinations of 0,1 in array of size 6
def tests():
  tests = product('01', repeat=6)
  tests = (list(tests))
  ret = []
  for i in tests:
    l = []
...
def run_tests(node, t):
  ...
  for test in t:
    ...
    if test[0] == 0 and test[1] == 0 and ((result == 1 and test[2] == 1) or (result == 0 and test[2] == 0)):
      fitness +=1
    elif test[0] == 1 and test[1] == 0 and ((result == 1 and test[3] == 1) or (result == 0 and test[3] == 0)):
      fitness +=1
    elif test[0] == 0 and test[1] == 1 and ((result == 1 and test[4] == 1) or (result == 0 and test[4] == 0)):
      fitness +=1
    elif test[0] == 1 and test[1] == 1 and ((result == 1 and test[5] == 1) or (result == 0 and test[5] == 0)):
      fitness +=1
  return fitness
...
def GP(num_iterations=100, size_pop=50):
  ...
  p = run_fitness(p, t)
  y = [sorted(p, key=lambda x: x[1], reverse=True)[0][1] / 64]
  best = y[0]
